...
import {
  Smartphone, CreditCard, DollarSign, Users,
  Plus, Lock, Unlock, AlertCircle, RefreshCw, Activity, Shield, LayoutDashboard,
} from 'lucide-react';
...
export default function DealerDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const [c, d, e, l] = await Promise.all([
        supabase.from('customers').select('*').eq('dealer_id', user.id).order('created_at', { ascending: false }),
        supabase.from('devices').select('*').eq('dealer_id', user.id).order('created_at', { ascending: false }),
        supabase.from('emi_schedules').select('*').eq('dealer_id', user.id).order('created_at', { ascending: false }),
        supabase.from('activity_logs').select('*').eq('user_id', user.id).order('created_at', { ascending: false }).limit(50),
      ]);
      if (c.error) throw c.error;
      setCustomers(c.data || []);
      setDevices(d.data || []);
      const emiList = e.data || [];
      setEmis(emiList);
      setLogs(l.data || []);

      // Load payments for this dealer's EMI schedules
      if (emiList.length > 0) {
        const emiIds = emiList.map((e: EmiSchedule) => e.id);
        const p = await supabase.from('payments').select('*').in('emi_schedule_id', emiIds).order('due_date', { ascending: true });
        setPayments(p.data || []);
      } else {
        setPayments([]);
      }
  ...
  async function addCustomer() {
    ...
    const { error } = await supabase.from('customers').insert({
      ...cust,
      dealer_id: user.id,
      // user_id is null — the customer doesn't have a login account by default
    });
  ...
  async function createEmi() {
    ...
    if (!total || !months || !emi.customer_id || !emi.device_id) { alert('Please fill all required fields'); return; }
    const { data: emiData, error } = await supabase
      ...
      .select().single();
    if (error || !emiData) { alert(error?.message || 'Failed to create EMI plan'); return; }
    ...
    <div className="flex min-h-screen bg-gray-50">
        ...
        <div className="p-6 lg:p-8 max-w-7xl mx-auto">
          ...
          {tab === 'overview' && (
            <div className="space-y-6">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { label: 'Customers', value: customers.length, icon: Users, color: 'bg-blue-100 text-blue-600' },
                  { label: 'Active Devices', value: devices.filter((d) => d.status === 'active').length, icon: Smartphone, color: 'bg-green-100 text-green-600' },
                  { label: 'Active EMIs', value: emis.filter((e) => e.status === 'active').length, icon: CreditCard, color: 'bg-orange-100 text-orange-600' },
                  { label: 'Collected', value: fmt(totalCollected), icon: DollarSign, color: 'bg-primary-100 text-primary-600' },
                ].map((s) => {
                  const Icon = s.icon;
                  return (
              ...
              <div className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm">
                <h3 className="text-base font-semibold text-gray-900 mb-4 flex items-center gap-2"><LayoutDashboard className="w-4 h-4 text-primary-600" />Quick Actions</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <button onClick={() => setShowCust(true)} className="p-4 bg-blue-50 rounded-xl border border-blue-100 text-blue-700 text-sm font-medium hover:bg-blue-100 transition">Add Customer</button>
                  <button onClick={() => setShowDev(true)} className="p-4 bg-green-50 rounded-xl border border-green-100 text-green-700 text-sm font-medium hover:bg-green-100 transition">Register Device</button>
                  <button onClick={() => setShowEmi(true)} className="p-4 bg-orange-50 rounded-xl border border-orange-100 text-orange-700 text-sm font-medium hover:bg-orange-100 transition">Create EMI Plan</button>
                  <button onClick={() => setTab('payments')} className="p-4 bg-primary-50 rounded-xl border border-primary-100 text-primary-700 text-sm font-medium hover:bg-primary-100 transition">View Collections</button>
                </div>
              </div>
              <div className="grid md:grid-cols-2 gap-4">
                <div className="bg-green-50 rounded-xl p-5 border border-green-200"><p className="text-sm text-green-700 font-medium">Total Collected</p><p className="text-2xl font-bold text-green-600 mt-1">{fmt(totalCollected)}</p></div>
                <div className="bg-red-50 rounded-xl p-5 border border-red-200"><p className="text-sm text-red-700 font-medium">Pending Amount</p><p className="text-2xl font-bold text-red-600 mt-1">{fmt(totalPending)}</p></div>
              </div>
            </div>
          )}
          ...
          {tab === 'customers' && (
            <div className="space-y-4">
              ...
              {customers.length === 0 ? (
                <div className="bg-white rounded-xl p-12 border border-gray-100 text-center"><Users className="w-12 h-12 text-gray-200 mx-auto mb-3" /><p className="text-gray-500">No customers yet.</p></div>
              ) : (
                <div className="bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm">
                  <table className="w-full text-sm">
                    <thead><tr className="border-b border-gray-100 bg-gray-50"><th className="text-left py-3 px-4 font-medium text-gray-500">Name</th><th className="text-left py-3 px-4 font-medium text-gray-500">Email</th><th className="text-left py-3 px-4 font-medium text-gray-500">Phone</th><th className="text-left py-3 px-4 font-medium text-gray-500">Company</th></tr></thead>
                    <tbody>{customers.map((c) => (<tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50 transition"><td className="py-3 px-4 font-medium text-gray-900">{c.first_name} {c.last_name}</td><td className="py-3 px-4 text-gray-600">{c.email}</td><td className="py-3 px-4 text-gray-600">{c.phone}</td><td className="py-3 px-4 text-gray-600">{c.company_name || '—'}</td></tr>))}</tbody>
                  </table>
                </div>
              )}
            </div>
              ...
              {devices.length === 0 ? (
                ...
                <div className="bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm">
                  <table className="w-full text-sm">
                    ...
                    <tbody>{devices.map((d) => (<tr key={d.id} className="border-b border-gray-50 hover:bg-gray-50 transition"><td className="py-3 px-4 font-medium text-gray-900">{d.device_name}</td><td className="py-3 px-4 text-gray-500 font-mono text-xs">{d.imei}</td><td className="py-3 px-4 text-gray-600">{custName(d.customer_id)}</td><td className="py-3 px-4"><StatusBadge status={d.status} /></td><td className="py-3 px-4">{d.status === 'active' && <button onClick={() => { setSelectedDev(d); setShowLock(true); }} className="flex items-center gap-1 px-3 py-1.5 bg-red-50 text-red-600 text-xs font-medium rounded-lg hover:bg-red-100 transition"><Lock className="w-3 h-3" />Lock</button>}{d.status === 'locked' && <button onClick={() => unlockDevice(d)} className="flex items-center gap-1 px-3 py-1.5 bg-green-50 text-green-600 text-xs font-medium rounded-lg hover:bg-green-100 transition"><Unlock className="w-3 h-3" />Unlock</button>}</td></tr>))}</tbody>
                  </table>
                </div>
              )}
            </div>
          ...
          {tab === 'emi' && (
            <div className="space-y-4">
              ...
              {emis.length === 0 ? (
                <div className="bg-white rounded-xl p-12 border border-gray-100 text-center"><CreditCard className="w-12 h-12 text-gray-200 mx-auto mb-3" /><p className="text-gray-500">No EMI plans yet. Add customers and register devices first.</p></div>
              ) : (
                <div className="bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm">
                  <table className="w-full text-sm">
                    ...
                    <tbody>{emis.map((e) => (<tr key={e.id} className="border-b border-gray-50 hover:bg-gray-50 transition"><td className="py-3 px-4 font-medium text-gray-900">{custName(e.customer_id)}</td><td className="py-3 px-4 text-gray-600">{devLabel(e.device_id)}</td><td className="py-3 px-4 font-medium">{fmt(Number(e.total_amount))}</td><td className="py-3 px-4 text-gray-600">{fmt(Number(e.total_amount) / e.duration_months)}</td><td className="py-3 px-4 text-gray-600">{e.duration_months} mo</td><td className="py-3 px-4"><StatusBadge status={e.status} /></td></tr>))}</tbody>
                  </table>
                </div>
              )}
            </div>
          ...
          {tab === 'payments' && (
            <div className="space-y-4">
              <h2 className="text-lg font-semibold text-gray-900">EMI Collection</h2>
              <div className="grid md:grid-cols-3 gap-4">
                ...
                <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm"><p className="text-sm text-gray-500">Overdue</p><p className="text-2xl font-bold text-orange-600 mt-1">{payments.filter((p) => p.status === 'overdue').length}</p></div>
              </div>
          ...
          {tab === 'lock' && (
            <div className="space-y-4">
              <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2"><Shield className="w-5 h-5 text-primary-600" />Remote Lock/Unlock</h2>
              <p className="text-sm text-gray-500">Secure devices remotely when EMI payments default.</p>
              {devices.length === 0 && <div className="bg-white rounded-xl p-8 border border-gray-100 text-center"><p className="text-gray-400">No devices registered.</p></div>}
              <div className="grid md:grid-cols-2 gap-4">
                {devices.filter((d) => d.status === 'active').map((d) => (
                  <div key={d.id} className="bg-white rounded-xl p-5 border border-gray-100 flex items-center justify-between shadow-sm">
                    <div><p className="font-medium text-gray-900">{d.device_name}</p><p className="text-xs font-mono text-gray-500 mt-0.5">{d.imei}</p><p className="text-xs text-gray-400">{custName(d.customer_id)}</p></div>
                ...
                {devices.filter((d) => d.status === 'locked').map((d) => (
                  <div key={d.id} className="bg-white rounded-xl p-5 border border-red-200 flex items-center justify-between shadow-sm">
                    <div><p className="font-medium text-gray-900">{d.device_name}</p><p className="text-xs font-mono text-gray-500 mt-0.5">{d.imei}</p><StatusBadge status="locked" /></div>
          ...
          {tab === 'activity' && (
            <div className="space-y-4">
              <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2"><Activity className="w-5 h-5 text-primary-600" />Account Activities</h2>
              {logs.length === 0 ? (
      ...
      <Modal isOpen={showCust} onClose={() => setShowCust(false)} title="Add New Customer">
        <form onSubmit={(e) => { e.preventDefault(); addCustomer(); }} className="space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-2"><button type="button" onClick={() => setShowCust(false)} className={BTN_CANCEL}>Cancel</button><button type="submit" className={BTN_PRIMARY}>Add Customer</button></div>
        </form>
      </Modal>
      ...
      <Modal isOpen={showDev} onClose={() => setShowDev(false)} title="Register Device">
        <form onSubmit={(e) => { e.preventDefault(); registerDevice(); }} className="space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-2"><button type="button" onClick={() => setShowDev(false)} className={BTN_CANCEL}>Cancel</button><button type="submit" className={BTN_PRIMARY}>Register</button></div>
        </form>
      </Modal>
      ...
      <Modal isOpen={showEmi} onClose={() => setShowEmi(false)} title="Create EMI Plan">
        <form onSubmit={(e) => { e.preventDefault(); createEmi(); }} className="space-y-3">
          ...
          {emi.total_amount && emi.duration_months && <div className="p-3 bg-primary-50 rounded-lg border border-primary-100 text-sm text-primary-700">Monthly EMI: <strong>{fmt(Number(emi.total_amount) / Number(emi.duration_months))}</strong></div>}
          <div className="flex justify-end gap-2 pt-2"><button type="button" onClick={() => setShowEmi(false)} className={BTN_CANCEL}>Cancel</button><button type="submit" className={BTN_PRIMARY}>Create EMI Plan</button></div>
        </form>
      </Modal>
      ...
      <Modal isOpen={showPay} onClose={() => setShowPay(false)} title="Record Payment">
        <form onSubmit={(e) => { e.preventDefault(); recordPayment(); }} className="space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-2"><button type="button" onClick={() => setShowPay(false)} className={BTN_CANCEL}>Cancel</button><button type="submit" className="px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 transition">Record Payment</button></div>
        </form>
      </Modal>

      <Modal isOpen={showLock} onClose={() => setShowLock(false)} title="Lock Device">
        <div className="space-y-4">
          <div className="flex items-start gap-2 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm"><AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5" />This will remotely lock the device. The customer will be unable to use it until unlocked.</div>
          {selectedDev && <div className="p-3 bg-gray-50 rounded-lg text-sm space-y-1"><p><span className="font-medium">Device:</span> {selectedDev.device_name}</p><p><span className="font-medium">IMEI:</span> <span className="font-mono">{selectedDev.imei}</span></p><p><span className="font-medium">Customer:</span> {custName(selectedDev.customer_id)}</p></div>}
          <textarea placeholder="Reason for locking (e.g. EMI default)..." value={lockReason} onChange={(e) => setLockReason(e.target.value)} className={INPUT + ' resize-none'} rows={3} />
          <div className="flex justify-end gap-2"><button onClick={() => setShowLock(false)} className={BTN_CANCEL}>Cancel</button><button onClick={lockDevice} className="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 transition">Lock Device</button></div>
        </div>
      </Modal>
    </div>
  );
